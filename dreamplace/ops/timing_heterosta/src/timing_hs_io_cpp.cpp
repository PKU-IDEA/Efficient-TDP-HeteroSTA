...
NetlistDB* TimingHeterostaIO::build_netlistdb_from_dreamplace(PlaceDB& placedb, 
	...
	try {
		...
		{
			dreamplacePrint(kDEBUG, "pin_idx=%zu, name=%s\n",    
			             pin_idx, g_netlist_data.pin_names[pin_idx].c_str());
		}
		*/
...
TimingHeterostaIO::STAHoldingsPtr TimingHeterostaIO::initialize_heterosta() {
	...
	if (lic == nullptr) {
        if (hardcode_lic == nullptr) {
           dreamplacePrint(kERROR, "[FATAL ERROR] License not found.\n");
           dreamplacePrint(kERROR, "               Please either set the 'HeteroSTA_Lic' environment variable\n");
           dreamplacePrint(kERROR, "               or provide a hardcoded license in the source code.\n");
            return  STAHoldingsPtr(nullptr, heterosta_free);
        }
        lic = hardcode_lic;
        dreamplacePrint(kWARN, "[INFO] 'HeteroSTA_Lic' environment variable not found. Using hardcoded license.\n");
    } else {
        dreamplacePrint(kINFO, "[INFO] Successfully loaded license from 'HeteroSTA_Lic' environment variable.\n");
	}
