...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    pybind11::class_<DREAMPLACE_NAMESPACE::TimerWrapper, std::shared_ptr<DREAMPLACE_NAMESPACE::TimerWrapper>>(m, "Timer")
        ...
        .def("dump_paths_setup_to_file", [](DREAMPLACE_NAMESPACE::TimerWrapper& wrapper, uintptr_t num_paths, 
                                            ...
                                            bool use_cuda) {
            heterosta_dump_paths_setup_to_file(wrapper.get_raw_timer(), num_paths, nworst, true, file_path.c_str(), use_cuda);
    ...
    m.def("update_net_weights", [](DREAMPLACE_NAMESPACE::TimerWrapper& wrapper, int n,
                                    ...
                                    int pin2pin_min_weight, double pin2pin_accumulate_weight,
                                    bool use_cuda) {

        // Convert Python dict to C++ unordered_map
        DREAMPLACE_NAMESPACE::Pin2PinWeightMap cpp_pin2pin_map;
        for (auto item : pin2pin_net_weight) {
            auto key_tuple = item.first.cast<pybind11::tuple>();
            long pin1 = key_tuple[0].cast<long>();
            long pin2 = key_tuple[1].cast<long>();
            float weight = item.second.cast<float>();
            cpp_pin2pin_map[std::make_pair(pin1, pin2)] = weight;
        }

        DREAMPLACE_NAMESPACE::TimingHeterostaCpp::update_net_weights(
            wrapper.get_raw_timer_ref(), n, num_nets, num_pins,
            flat_netpin, netpin_start, pin2node_map, pin2net_map, net_criticality, net_criticality_deltas,
            net_weights, net_weight_deltas, degree_map, max_net_weight, cpp_pin2pin_map,
            net_weighting_scheme, momentum_decay_factor,
            ignore_net_degree, pin2pin_max_weight, pin2pin_min_weight,
            pin2pin_accumulate_weight,
            use_cuda);

        // Convert back to Python dict
        pin2pin_net_weight.clear();
        for (const auto& pair : cpp_pin2pin_map) {
            pybind11::tuple key = pybind11::make_tuple(pair.first.first, pair.first.second);
            pin2pin_net_weight[key] = pair.second;
        }
       ...
       pybind11::arg("pin2node_map"),pybind11::arg("pin2net_map"),
       ...
       pybind11::arg("max_net_weight"), 
       pybind11::arg("pin2pin_net_weight"),
       pybind11::arg("net_weighting_scheme"),
       pybind11::arg("momentum_decay_factor"),
       pybind11::arg("ignore_net_degree"), 
       pybind11::arg("pin2pin_max_weight"),
       pybind11::arg("pin2pin_min_weight"),
       pybind11::arg("pin2pin_accumulate_weight"),
       pybind11::arg("use_cuda"));
